...
testresult_dir = '/data1/zhengkengtao/exps/0730_handcraft_methods/ADQ1/docimg_split811test/'
map_path = testresult_dir + 'Output_map/'
pred_path = testresult_dir +  'threshold_0.5/'
...
# # 算其它算法指标 ———— 篡改脱敏视为两类（这里计算指标有点逻辑bug，默认其它算法也能够区分脱敏和篡改，同时虚警的地方认为是篡改）
# gt_path = '/data1/zhengkengtao/docimg/docimg_split811/test_gt3/'
# pred_path = '/data1/zhengkengtao/exps/0723_MVSSNet_docimg_split811_png/test_docimgsplit811test_epoch63_crop512stride256/pred_512x512/predblockmerge_thres0.5/'
# save_metrics_path = '/data1/zhengkengtao/exps/0723_MVSSNet_docimg_split811_png/test_docimgsplit811test_epoch63_crop512stride256/pred_512x512/'
# print(len(os.listdir(pred_path)))
# metrics_csv = save_metrics_path +  "forgerymosaic_samekind_SplitMetrics.csv"
# f = open(metrics_csv, 'a+', newline='')
# csv_writer = csv.writer(f)
# csv_writer.writerow(['img','gt','f1-1','iou-1','mcc-1','fpr-1','f1-2','iou-2','mcc-2','fpr-2'])
# f.close()
# filenames = os.listdir(pred_path)
# filenames.sort()
# f1forgerys, iouforgerys, mccforgerys, fprforgerys, f1mosaics, ioumosaics, mccmosaics, fprmosaics= [], [], [], [], [], [], [], []
# for i in range(len(filenames)):
#     name = filenames[i]
#
#     pred = Image.open(pred_path + name).convert('L')
...
#     predmosaic = predall - predforgery
#
#     tpr_recall1, tnr1, fpr1, fnr1, precision1, f11, mcc1, iou1, tn1, tp1, fn1, fp1 = get_metrics_without_auc(gtforgery, predforgery)
#     tpr_recall2, tnr2, fpr2, fnr2, precision2, f12, mcc2, iou2, tn2, tp2, fn2, fp2 = get_metrics_without_auc(gtmosaic, predmosaic)
#
#     print('{}, {}, f1forgery:{:.3f}, iouforgery:{:.3f}, mccforgery:{:.3f}, fprforgery:{:.3f}, '
#                 'f1mosaic:{:.3f}, ioumosaic:{:.3f}, mccmosaic:{:.3f}, fprmosaic:{:.3f}'
#                 .format(i, name, f11, iou1, mcc1, fpr1, f12, iou2, mcc2, fpr2))
#     print('======================================================================================================')
#     f = open(metrics_csv, 'a+', newline='')
#     csv_writer = csv.writer(f)
#     csv_writer.writerow([name, gt_name, f11, iou1, mcc1, fpr1, f12, iou2, mcc2, fpr2])
#     f.close()
#     f1forgerys.append(f11)
#     iouforgerys.append(iou1)
#     mccforgerys.append(mcc1)
...
#     fprmosaics.append(fpr2)
#
# f1forgery_mean, iouforgery_mean, mccforgery_mean, fprforgery_mean = np.mean(f1forgerys), np.mean(iouforgerys), np.mean(mccforgerys), np.mean(fprforgerys)
# f1mosaic_mean, ioumosaic_mean, mccmosaic_mean, fprmosaic_mean= np.mean(f1mosaics), np.mean(ioumosaics), np.mean(mccmosaics), np.mean(fprmosaics)
# print('======================================================================================================')
# print('test num:{}, f1foregry:{:.3f}, iouforgey:{:.3f}, mccforgery:{:.3f}, fprforgery:{:.3f}, '
#             'f1mosaic:{:.3f}, ioumosaic:{:.3f}, mccmosaic:{:.3f}, fprmosaic:{:.3f}'
#             .format(len(filenames), f1forgery_mean, iouforgery_mean, mccforgery_mean, fprforgery_mean, f1mosaic_mean, ioumosaic_mean, mccmosaic_mean, fprmosaic_mean))
# f = open(metrics_csv, 'a+', newline='')
# csv_writer = csv.writer(f)
# csv_writer.writerow(['END','END','f1-1','iou-1','mcc-1','fpr-1','f1-2','iou-2','mcc-2','fpr-2'])
# csv_writer.writerow(['Average', len(filenames), f1forgery_mean, iouforgery_mean, mccforgery_mean, fprforgery_mean, f1mosaic_mean, ioumosaic_mean, mccmosaic_mean, fprmosaic_mean])
# f.close()