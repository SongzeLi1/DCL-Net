...
from FAST_utils import read_dataset
from shutil import rmtree
from tee_print import TeePrint
from FAST_utils import MyDataset, time_calucate, data_path
from loss_function import DiceLoss, iou_compute
from sklearn.metrics import f1_score
from models.FASTnet import InpaintGenerator
...
def visual(test_loader,mode):
    ...
    with torch.no_grad():
        for step, data in enumerate(test_loader):
            ...
            print('save image:', os.path.join(output_root, names[0].split('/')[-1].replace('.jpg', '.png')))
            # for i in range(b):
...
def test(test_loader):
    ...
    with torch.no_grad():
        ...
        for step, data in enumerate(test_loader):
            images, labels, fres, _ = data
            images, labels, fres = images.to(device), labels.to(device), fres.to(device)
            images = images.permute([0,1,4,2,3])  #b, t, c, h, w
            labels = labels.long()
            outputs = model(images, fres)
            outputs = torch.sigmoid(outputs).cuda()
            pred = outputs.permute([0, 2, 3, 1])
            ...
            if np.isnan(fpr_) == False:
                fpr += fpr_
            total += 1
            sys.stdout.write('\r->Iter [%d/%d], loss: %.4f, acc: %.4f, \033[1;31mf1: %.4f\033[0m, iou：%.4f, fpr：%.4f'
...
def train(train_loader, valid_loader_VI,valid_loader_OP,valid_loader_CP, optimizer, current_epoch, print_func):
    ...
    for epoch in range(args.Epoch)[current_epoch:]:
        ...
        if epoch %10==0 and epoch !=0:
            print('\nWaiting val...')
            val_loss, val_acc, val_f1, val_iou, _ = test(valid_loader_VI)
            val_loss, val_acc, val_f1, val_iou, _ = test(valid_loader_OP)
            val_loss, val_acc, val_f1, val_iou, _ = test(valid_loader_CP)
            if best_f1 < val_f1:
                best_f1 = val_f1
...
def main():
        ...
        if not os.path.isdir(save_path):
            ...
            if os.listdir(save_path):
                ...
                if (chioce == 'y' or chioce == 'Y'):
                    ...
                    return None
        tee_print = TeePrint(filename=save_path + '.log', mode=write_log_mode)
    ...
    if args.mode == 'train':
        ...
        train_data = read_dataset(train_root, mode='train', pattern='*.png',print_func=print_func)
        valid_data_VI = read_dataset([valid_root[0]], mode='val', pattern='*.png',print_func=print_func)
        valid_data_OP = read_dataset([valid_root[1]], mode='val', pattern='*.png',print_func=print_func)
        valid_data_CP = read_dataset([valid_root[2]], mode='val', pattern='*.png',print_func=print_func)
        ...
        valid_imgs_VI, valid_labels_VI = valid_data_VI
        valid_imgs_OP, valid_labels_OP = valid_data_OP
        valid_imgs_CP, valid_labels_CP = valid_data_CP

        train_data = MyDataset(imgs=train_imgs, labels=train_labels, resize=resize)
        valid_data_VI = MyDataset(imgs=valid_imgs_VI, labels=valid_labels_VI, resize=resize)
        valid_data_OP = MyDataset(imgs=valid_imgs_OP, labels=valid_labels_OP, resize=resize)
        valid_data_CP = MyDataset(imgs=valid_imgs_CP, labels=valid_labels_CP, resize=resize)
        ...
        valid_loader_VI = DataLoader(dataset=valid_data_VI, batch_size=1, shuffle=False, drop_last=True, num_workers=4, pin_memory=True)
        valid_loader_OP = DataLoader(dataset=valid_data_OP, batch_size=1, shuffle=False, drop_last=True, num_workers=4, pin_memory=True)
        valid_loader_CP = DataLoader(dataset=valid_data_CP, batch_size=1, shuffle=False, drop_last=True, num_workers=4,pin_memory=True)
        train(train_loader, valid_loader_VI, valid_loader_OP, valid_loader_CP, optimizer, current_epoch, print_func=print_func)
    elif args.mode in ['test', 'visual']:
        test_data = read_dataset(test_root, mode=args.mode, pattern='*.png', print_func=print_func)
        test_imgs, test_labels = test_data
        test_data = MyDataset(imgs=test_imgs, labels=test_labels,resize=resize)
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--data_root", type=str, default='/pubdata/weishujin/Ali/train_part/crop/512_512/image')
    parser.add_argument("--data_root", type=str, default='VI_OP')
    parser.add_argument('--restore_path', type=str, default=None)
    args = parser.parse_args()

    device = torch.device('cuda')

    # mvssnet_path = '/home/weishujin/Codes/pytorch/MVSS-Net-master/ckpt/mvssnet_tianchi.pt'
    ...
    inapinting_root = '/data/wanghao/Datasets/inpainting_data/train/png/&/*'  #文件路径
    roots = (args.data_root).split('_')
    train_root = []
    for r in roots:
        train_root.append(inapinting_root.replace('&', r, 1))
        train_root.append(inapinting_root.replace('train','val',1).replace('&', r, 1))
        valid_root = [inapinting_root.replace('train','test',1).replace('&','VI',1),
                      inapinting_root.replace('train','test',1).replace('&','OP',1),
                      inapinting_root.replace('train','test',1).replace('&','CP',1)]
        # test_root = ['/data/weishujin/Inpainting_dataset/Compression/h264/CQP23/FFIT/test/png/*/*']
        # test_root = ['/data/weishujin/Inpainting_dataset/Compression/h264/Origin/inpainting_data/*/png/CP/*']
        test_root = ['/data/weishujin/Inpainting_dataset/Compression/h264/CQP30/FFIT/test/png/*/*']
        print(train_root)
        print(valid_root)
        print(test_root)

    save_path = os.path.join('/home/weishujin/Codes/Pytorch_codes/MVSS-Net-master/save/VI_OP')
    main()
