...
from utils_nclass3 import read_dataset
from shutil import rmtree
from tee_print import TeePrint
from utils_nclass3 import MyDataset, time_calucate, data_path
from loss_function import iou_compute
from segmentation_models_pytorch.losses import DiceLoss
from sklearn.metrics import f1_score
from models.mvssnet_nclass3 import get_mvss
...
class DiceLoss_edge(nn.Module):
    def __init__(self):
        super(DiceLoss_edge, self).__init__()
...
def test(test_loader, mode='joint', visual='N'):
    ...
    with torch.no_grad():
        ...
        for step, data in enumerate(test_loader):
            ...
            pred_map = F.softmax(pred, dim=1)
            # pred_map[pred_map >= 0.5] = 1
            # pred_map[pred_map < 0.5] = 0
            pred_map = torch.argmax(pred_map, dim=1)
            print('predmap:', pred_map.shape, pred_map.min(), pred_map.max())
...
def train(train_loader, valid_loader, optimizer, current_epoch, print_func):
    ...
    for epoch in range(args.Epoch)[current_epoch:]:
        ...
        for step, data in enumerate(train_loader):
            ...
            # print('^^^^:', labels.min(),labels.max(),edg_labels.min(),edg_labels.max())
            images, labels = images.to(device), labels.to(device)
            b, h, w, c = labels.shape
            ...
            if args.use_sobel == True:
                ...
                edg_loss = criterion_edge(res1, edg_labels)
            ...
            optimizer.step()
            epoch_loss += loss.item()
            pred_map = torch.argmax(outputs, axis=1)
            # print('^^^^:', pred_map.shape, pred_map.min(),pred_map.max())
            correct += torch.eq(pred_map, labels).float().mean().item()
            total += 1
...
def main():
    ...
    if args.mode == 'train':
        ...
        # train_imgs = train_imgs[:72]
        # train_labels = train_labels[:72]
        # valid_imgs = valid_imgs[:3]
        ...
        # 数据增强
        train_data = MyDataset(imgs=train_imgs, labels=train_labels, use_sobel=args.use_sobel, resize=resize, transform=train_transform(input_size=resize))
        valid_data = MyDataset(imgs=valid_imgs, labels=valid_labels, use_sobel=args.use_sobel, resize=resize, transform=None)
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--data_root", type=str, default='/data1/zhengkengtao/docimg/docimg_split811/crop512x512/patch_noblank/train_images')
    # parser.add_argument("--data_root", type=str, default='/data1/zhengkengtao/docimg/docimg_split811/crop512x512/train_select/patch_noblank+blankhalf/train_images')
    # parser.add_argument("--data_root", type=str, default='/data1/zhengkengtao/Ali_new/forgery_round1_train_20220217/train_split811/train_imgs')
    parser.add_argument("--data_root", type=str, default='/data1/zhengkengtao/docimg/docimg_split811/crop512x512/train_select/patch_noblank/train_images')
    ...
    if args.loss == 'dice':
        ...
        criterion_edge = DiceLoss_edge().to(device)

    train_root = [args.data_root]
    # valid_root = ['/data1/zhengkengtao/docimg/docimg_split811/crop512x512/patch_noblank/val_images']
    # valid_root = ['/data1/zhengkengtao/docimg/docimg_split811/crop512x512/val_select/patch_noblank/val_images']
    # valid_root = ['/data1/zhengkengtao/Ali_new/forgery_round1_train_20220217/train_split19/test_imgs']
    # valid_root = ['/data1/zhengkengtao/Ali_new/forgery_round1_train_20220217/train_split811/val_imgs']
    valid_root = ['/data1/zhengkengtao/docimg/docimg_split811/crop512x512/val_select_noblank/val_images']
    test_root = None
    print(train_root)
    print(valid_root)
    print(test_root)

    train_resize = (512, 512) # (768, 768) None (512, 512)
    resize = train_resize
    print('resize:', resize)

    save_path = os.path.join('/data1/zhengkengtao/exps/0728_MVSSNet_docimg_split811_crop512noblank_nclass3_Aug_1/')
    main()
