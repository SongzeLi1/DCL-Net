...
def read_dataset(args, data_dir, mode,pattern ,print_func,shuffle_seed=None):
    ...
    if 'Inpainting_dataset' in data_dir[0]:
        ...
        if mode != 'visual':
            if 'tamper' in data_dir[0]:
                labels = [img.replace('tamper', 'tamper_gt3', 1).replace('psc', 'gt3', 1) for img in imgs]
                # print(labels)
            else:
...
class MyDataset(Dataset): #创建自己的类：MyDataset,这个类是继承的torch.utils.data.Dataset
    ...
    def __getitem__(self, index):
        ...
        if labels_path != None:
            ...
            # print(imgs_.shape) # [c, h, w]
            h, w = imgs_.shape[-2], imgs_.shape[-1]
            labels_ = np.zeros((h, w), np.int32)
            # print(labels_.shape) # [h, w]
            labels_ = labels_[..., np.newaxis]
            # print(labels_.shape) # [h, w]
            if self.use_sobel != None:
                edg_labels_ = np.zeros([h, w], dtype=np.uint8)
                # _, binary = cv2.threshold(label, 127, 255, cv2.THRESH_BINARY)
                # contours, _ = cv2.findContours(binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                # cv2.drawContours(edg_label, contours, -1, (255, 255, 255), 3)
                edg_labels_ = edg_labels_[..., np.newaxis]
                edg_labels_ = edg_labels_ / 255.
                edg_labels_ = cv2.resize(edg_labels_, (h // 4, w // 4))
                edg_labels_ = edg_labels_.astype(np.int32)
            else:
                edg_labels_ = 0
            imglevel_label = 0.
