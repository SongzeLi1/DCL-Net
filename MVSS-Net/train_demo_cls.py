...
from utils_cls import read_dataset
from shutil import rmtree
from tee_print import TeePrint
from utils_cls import MyDataset, time_calucate, data_path
from loss_function import DiceLoss, iou_compute
from sklearn.metrics import f1_score
from models.mvssnet_cls import get_mvss
...
def get_f1_iou_fpr(y_true, y_pred):
...
def test(test_loader, mode='joint', visual='N'):
    model.eval()
    with torch.no_grad():
        epoch_loss, correct, total, f1, iou, fpr, correct_cls = 0, 0, 0, 0, 0, 0, 0
        for step, data in enumerate(test_loader):
            images, labels, _, names, imglevel_labels = data
            images, labels = images.to(device), labels.to(device)
            imglevel_labels = imglevel_labels.to(device)
            b, h, w, c = labels.shape
            labels = labels.long()
            imglevel_labels = imglevel_labels.to(torch.float32)
            imglevel_labels = imglevel_labels.unsqueeze(0)
            # images = images.permute([0, 3, 1, 2])
            if mode == 'joint':
                assert b==1, 'Plese set batch size 1'
                images = images.permute([0, 2, 3, 1])
                ...
                images = torch.Tensor(images).to(device)
                images = images.permute([0, 3, 1, 2])
            # _, outputs, clspreds = model(images,train_mode='test')
            _, outputs, clspreds = model(images)
            outputs = torch.sigmoid(outputs).to(device)
            clspreds = clspreds.to(device) # 不用sigmoid
            if mode == 'joint':
                pred = outputs.permute([0, 2, 3, 1])
                pred = pred.cpu().detach().numpy()
                pred = pred.squeeze(-1)
                ...
                pred = outputs.permute([0, 2, 3, 1])
            loss = criterion(pred, labels) + criterion_cls(clspreds, imglevel_labels)
            ...
            clspreds_map = clspreds
            clspreds_map[clspreds_map >= 0.5] = 1
            clspreds_map[clspreds_map < 0.5] = 0
            correct_cls += torch.eq(clspreds_map, imglevel_labels).float().mean().item()
            ...
            f1_, iou_, fpr_ = get_f1_iou_fpr(labels.reshape([-1]).astype('int'), pred_map.reshape([-1]).astype('int'))
            ...
            if visual == 'Y':
                ...
                sys.stdout.write('->Iter [%d/%d], loss: %.4f, acc: %.4f, cls_acc: %.4f, \033[1;31mf1: %.4f\033[0m, iou：%.4f, fpr：%.4f'
                                 % (total, len(test_loader), epoch_loss / total, correct / total, correct_cls / total, f1 / total, iou / total,
                                 fpr / total))
                sys.stdout.write('\n')
        # return round(epoch_loss/total, 3), round(correct/total, 3), round(f1/total, 3), round(iou/total, 3),  round(fpr/total, 3)
        return round(epoch_loss/total, 3), round(correct/(total//2), 3), round(f1/(total//2), 3), round(iou/(total//2), 3),  round(fpr/(total//2), 3), round(correct_cls/(total), 3)
...
def train(train_loader, valid_loader, optimizer, current_epoch, print_func):
    ...
    for epoch in range(args.Epoch)[current_epoch:]:
        ...
        epoch_loss, epoch_out_loss, epoch_edg_loss, epoch_cls_loss, correct, total = 0, 0, 0, 0, 0, 0
        for step, data in enumerate(train_loader):
            star_time = time.time()
            images, labels, edg_labels, _, imglevel_label = data
            images, labels = images.to(device), labels.to(device)
            imglevel_label = imglevel_label.to(device)  # [b]
            imglevel_label = imglevel_label.to(torch.float32)
            imglevel_label_flag = imglevel_label.clone()
            imglevel_label = imglevel_label.unsqueeze(0)
            # print(imglevel_label)
            # print(imglevel_label.shape)  # [1, b]
            # print(labels.shape)  # [b]
            b, h, w, c = labels.shape
            labels = labels.reshape([-1, h, w]).long()
            optimizer.zero_grad()
            res1,outputs,cls_pred = model(images)
            ...
            if args.use_sobel == True:
                ...
                # print(imglevel_label_flag.shape)
                # print(imglevel_label)
                # print(pred.shape)
                # print(labels.shape)
                pred = pred[imglevel_label_flag != 0]
                labels = labels[imglevel_label_flag != 0]
                res1 = res1[imglevel_label_flag != 0]
                edg_labels = edg_labels[imglevel_label_flag != 0]
                # print(pred.shape)
                # print(labels.shape)
                if pred.shape[0] != 0:
                ...
                pred = pred[imglevel_label_flag != 0]
                labels = labels[imglevel_label_flag != 0]
                loss = args.a_lamda * criterion(pred, labels)
            cls_pred = cls_pred.permute(1, 0)
            cls_loss = criterion_cls(cls_pred, imglevel_label)
            epoch_cls_loss += cls_loss.item()
            loss = loss + (1 - args.a_lamda - args.b_lamda) * cls_loss
            loss.backward()
            optimizer.step()
            epoch_loss += loss.item()
            pred_map = outputs
            ...
            if args.use_sobel == True:
                sys.stdout.write('\r->Epoch [%d/%d], Iter [%d/%d], Loss: %.4f, out_loss: %.4f, edg_loss: %.4f, cls_loss: %.4f, Acc: %.4f, Lr：%.1e, Present_Time：%02d:%02d, Total_Time：%01d天%02d小时%02d分钟'
                                 % (epoch + 1, args.Epoch, total, len(train_loader), epoch_loss/total, epoch_out_loss/total, epoch_edg_loss/total, epoch_cls_loss/total, correct/total, optimizer.param_groups[0]['lr'],
            ...
            if step % (len(train_loader)//10) == 0 and step != 0:
                print('')
                print_func('->Epoch [%d/%d], Iter [%d/%d], Loss: %.4f, Acc: %.4f'% (epoch + 1, args.Epoch, total, len(train_loader), epoch_loss/total, correct/total))

        print('\nWaiting val...')

        val_loss, val_acc, val_f1, val_iou, _, val_cls_acc = test(valid_loader, mode='origin')

        # 保存最近模型
        state = {'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}
        latest_ckpt_full_path = os.path.join(save_path, 'latest-ckpt-full.pth')
        ...
        if best_clsacc < val_cls_acc:
            best_clsacc = val_cls_acc
            torch.save(model.state_dict(), save_path + '/' + str(epoch + 1) + '_acc_' + str(val_acc) + '_clsacc_' + str(val_cls_acc) + '_lr_' + str(args.lr) + '.pkl')
        if  best_f1_and_clsacc < val_f1 + val_cls_acc:
            best_f1_and_clsacc = val_f1 + val_cls_acc
            torch.save(model.state_dict(), save_path + '/' + str(epoch + 1) + '_acc_' + str(val_acc) + '_f1_' + str(val_f1) + '_clsacc_' + str(val_cls_acc) + '_lr_' + str(args.lr) + '.pkl')
        print_func('->Epoch [%d/%d], val_loss: %.4f, val_acc: %.3f, \033[1;31mval_f1: %.3f\033[0m, val_iou: %.3f, val_cls_acc: %.3f'
                     % (epoch + 1, args.Epoch, val_loss, val_acc, val_f1, val_iou, val_cls_acc))
        scheduler.step()
...
def main():
    ...
    print_func('resize: %s' % str(resize))
    print_func('-----------------------------------------')
    ...
    if args.mode == 'train':
        ...
        train_data_1 = read_dataset(args, train_root_1, mode='train', pattern='*.png', print_func=print_func)
        train_data_2 = read_dataset(args, train_root_2, mode='train', pattern='*.png', print_func=print_func)
        valid_data = read_dataset(args, valid_root, mode='val', pattern='*.png',print_func=print_func)
        valid_data_1 = read_dataset(args, valid_root_1, mode='val', pattern='*.png',print_func=print_func)
        valid_data_2 = read_dataset(args, valid_root_2, mode='val', pattern='*.png', print_func=print_func)
        ...
        print_func('train_tamper_root: %s' % train_root)
        print_func('train_real_root: %s' % train_root_1)
        print_func('train_real_root: %s' % train_root_2)
        print_func('val_tamper_root: %s' % valid_root)
        print_func('val_real_root: %s' % valid_root_1)
        print_func('val_real_root: %s' % valid_root_2)
        print_func('test_root: %s' % test_root)

        train_imgs, train_labels = train_data
        train_imgs_1, train_labels_1 = train_data_1
        train_imgs_2, train_labels_2 = train_data_2
        train_imgs = train_imgs + train_imgs_1 + train_imgs_2
        train_labels = train_labels + train_labels_1 + train_labels_2
        ...
        valid_imgs_1, valid_labels_1 = valid_data_1
        valid_imgs_2, valid_labels_2 = valid_data_2
        valid_imgs = valid_imgs + valid_imgs_1 + valid_imgs_2
        valid_labels = valid_labels + valid_labels_1 + valid_labels_2

        # train_imgs = train_imgs[:36*2]
        # train_labels = train_labels[:36*2]
        # valid_imgs = valid_imgs[:36*2]
        # valid_labels = valid_labels[:36*2]
        print_func('train nums:{} val nums:{}'.format(len(train_imgs), len(valid_imgs)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--a_lamda", type=float, default=0.16)
    ...
    parser.add_argument("--data_root", type=str, default='/data1/zhengkengtao/docimg/docimg_split811/crop512x512/select/train_tamper')
    parser.add_argument("--data_root_1", type=str, default='/data1/zhengkengtao/docimg/docimg_split811/crop512x512/select/train_real')
    parser.add_argument("--data_root_2", type=str, default='/data1/zhengkengtao/docimg/docimg_split811/train_images_orig_crop512stride512')
    ...
    train_root_1 = [args.data_root_1]
    train_root_2 = [args.data_root_2]
    valid_root = ['/data1/zhengkengtao/docimg/docimg_split811/crop512x512/select/val_tamper']
    valid_root_1 = ['/data1/zhengkengtao/docimg/docimg_split811/crop512x512/select/val_real']
    valid_root_2 = ['/data1/zhengkengtao/docimg/docimg_split811/val_images_orig_crop512stride512']
    test_root = None

    train_resize = None # 必须为None
    resize = train_resize

    save_path = os.path.join('/data1/zhengkengtao/exps/1427_mvss_docimgsplit811_crop512train_Cls_noAug_adddata/')
    main()
