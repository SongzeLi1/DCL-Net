...
class MVSSNet(ResNet50):
    ...
    def forward(self, x, train_mode='train'):
        size = x.size()[2:]
        input_ = x.clone()
        feature_map, _ = self.base_forward(input_)
        c1, c2, c3, c4 = feature_map

        if (train_mode=='train') and self.sobel:
        ...
        # print(x0.max())
        cls = torch.sigmoid(x0)
        # print(cls.shape)
        b, c, h, w = cls.size()
        gmp = torch.nn.AdaptiveMaxPool2d((1, 1))
        # print(cls.min(), cls.max())
        cls = gmp(cls).view(b, -1)
        # print(cls.shape, cls)
        # print(x0.max())
