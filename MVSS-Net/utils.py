...
def read_dataset(args, data_dir, mode,pattern ,print_func,shuffle_seed=None):
    ...
    if 'Inpainting_dataset' in data_dir[0]:
        ...
        if mode != 'visual':
            if pattern == '*.png': # docimg png
                # labels = [img.replace('images', 'gt3_01', 1).replace('psc', 'gt3', 1) for img in imgs]
                labels = [img.replace('images', 'gt3_01_OrigMosaicOneKind', 1).replace('psc', 'gt3', 1) for img in imgs]
            else:
