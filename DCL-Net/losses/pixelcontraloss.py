...
def contrastive(feats_, labels_):
    anchor_num, n_view = feats_.shape[0], feats_.shape[1]
    ...
    anchor_dot_contrast = torch.div(torch.matmul(anchor_feature, torch.transpose(contrast_feature, 0, 1)), 0.6)
    ...
    loss = - (0.6 / 1) * mean_log_prob_pos
    loss = loss.mean()

    return loss

def PixelContrastLoss(feats, labels=None, predict=None):
    labels = labels.unsqueeze(1).float().clone()
    print(labels.shape)
    labels = F.interpolate(labels, (feats.shape[2], feats.shape[3]), mode='nearest')
    labels = labels.squeeze(1).long()
    ...
    feats = feats.permute(0, 2, 3, 1)
    feats = feats.contiguous().view(feats.shape[0], -1, feats.shape[-1])
    loss = contrastive(feats, labels)
    return loss


if __name__ == '__main__':
    feats = torch.randn(2, 1, 4, 4)

    labels = torch.zeros(2, 4, 4)
    labels[:, 0:4, 2:5] = 1
    labels[:, 5:13, 9:15] = 2
    # predict = torch.randn(2, 1, 16, 16)
    # predict[0 <= predict < 0.3] = 0
    # predict[0.3 <= predict < 0.6] = 1
    # predict[predict >= 0.6] = 2

    loss = PixelContrastLoss(feats, labels)
    print(loss)