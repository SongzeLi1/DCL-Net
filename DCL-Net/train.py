...
def train_net(fold, logger, param, model, train_data, valid_data):
    ...
    for epoch in range(epoch_start, epochs):
        ...
        train_epoch_focalloss = AverageMeter()
        ...
        for batch_idx, batch_samples in enumerate(train_loader):
            ...
            train_epoch_focalloss.update(loss_focal.item())
        ...
        with torch.no_grad():
            f1forgerys, iouforgerys, f1mosaics, ioumosaics = [], [], [], []
            for batch_idx, batch_samples in enumerate(valid_loader):
                datas, gt3s = batch_samples['image'].cuda(), batch_samples['label'].cuda()
                gtforgerys, gtmosaics = gt3s.clone(), gt3s.clone()
                gtforgerys[gtforgerys == 2] = 0
                gtmosaics[gtmosaics == 1] = 0
                gtmosaics[gtmosaics == 2] = 1
                gtforgerys = gtforgerys.to(torch.int64)
                gtmosaics = gtmosaics.to(torch.int64)
                predicts, contra_predicts = model(datas)
        ...
        logger.info('[val] fold:{} epoch:{} f1foregry:{:.4f} iouforgey:{:.4f} f1mosaic:{:.4f} ioumosaic:{:.4f} score:{:.4f}'
                    .format(fold, epoch, f1forgery_mean, iouforgery_mean, f1mosaic_mean, ioumosaic_mean, score))
        ...
        torch.save(state, latest_ckpt_full_path)
...
if __name__ == '__main__':
    ...
    param['save_train_dir'] = '/pubdata/1/exps/'
    ...
    train_names = glob.glob('/pubdata/1/docimg/docimg_split811/crop512x512/train_images/*.png')
    train_names.sort()
    val_names = glob.glob('/pubdata/1/docimg/docimg_split811/crop512x512/val_images/*.png')
    val_names.sort()
