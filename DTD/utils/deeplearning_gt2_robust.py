...
def train_net(fold, logger, param, model, train_data, valid_data, device='cuda'):
    ...
    for epoch in range(epoch_start, epochs):
        ...
        for batch_idx, batch_samples in enumerate(train_loader):
            data, gt, dct, qs = batch_samples['image'], batch_samples['label'], batch_samples['dct'], batch_samples[
                'qs']
            data, target, dct_coef, qs = Variable(data.to(device)), Variable(gt.to(device)), Variable(
                dct.to(device)), Variable(qs.unsqueeze(1).to(device))
            ...
            with autocast(): #need pytorch > 1.6
                ...
                if param['mixup']:
                    ...
                    # loss_ce = SoftCrossEntropy_fn(pred, gt) # gt需要转化为float
                    # loss_ce_3 = F.cross_entropy(pred, gt) # RRUNet原论文的loss

                # loss = 3 * loss_dice + loss_ce
                # loss = loss_focal + loss_lovas
                loss = loss_lov + loss_ce
                # loss = loss_ce_3
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_samples in enumerate(valid_loader):
                datas, gt3s, dct2, qs2 = batch_samples['image'], batch_samples['label'], batch_samples['dct'], \
                batch_samples['qs']
                datas, gt3s, dct_coef2, qs2 = Variable(datas.to(device)), Variable(gt3s.to(device)), Variable(
