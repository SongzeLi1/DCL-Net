...
from dataset.DOCDataset_gt2s_type1 import DOCDataset
...
from utils.deeplearning_gt2_type1 import *
import warnings
...
param['n_class'] = 3
param['mixup'] = False
param['input_size'] = [256, 256] # width, height [1664, 1664], [1440, 1440] [1024, 1024]
param['epochs'] = 200        # 训练轮数，请和scheduler的策略对应，不然复现不出效果，对于t0=3,t_mut=2的scheduler来讲，44的时候会达到最优
param['batch_size'] = 8*20     # 批大小
param['disp_inter'] = 1      # 显示间隔(epoch)`
param['iter_inter'] = 30  #30     # 显示迭代间隔(batch)
param['min_inter'] = 10
...
param['save_train_dir'] = '/pubdata/lisongze/DCLNet/result/DTDNet_crop512x512/Alinew_finetune/'
...
param['pretrain_ckpt'] = "/pubdata/lisongze/DCLNet/result/DTDNet_crop512x512/detection_type1/dtd_512x512/fold_0_best-ckpt.pth" # "/pubdata/lisongze/DCLNet/result/DTDNet_crop512x512/detection_type1/dtd_512x512/latest-ckpt.pth"
...
# 准备数据集
data_dir = '/pubdata/lisongze/docimg/exam/docimg2jpeg/Ali_new/split_1000_200_2800/'#'/pubdata/zhengkengtao/docimg/docimg_split811/crop512x512/patch_noblank/'
train_imgs_dir = '/pubdata/lisongze/docimg/exam/docimg2jpeg/Ali_new/split_1000_200_2800/train_imgs_crop256stride128/'#'/pubdata/lisongze/docimg/exam/docimg2jpeg/train_images_75_100/'
# train_imgs_dir = os.path.join(data_dir, "train_images/")
train_labels_dir = os.path.join(data_dir, "train_gt3_crop256stride128/")
val_imgs_dir = "/pubdata/lisongze/docimg/exam/docimg2jpeg/Ali_new/split_1000_200_2800/val_imgs_crop256stride128/"#'/pubdata/lisongze/docimg/exam/docimg2jpeg/val_images_75_100/'
# val_imgs_dir = os.path.join(data_dir, "val_images/") # val_images
val_labels_dir = os.path.join(data_dir, "val_gt3_crop256stride128/") # val_gt3
...
# model = DIF()
model = seg_dtd('', 3)  # 'backbone' num_class
model = torch.nn.DataParallel(model)
input_tensor = torch.randn(1, 3, 512, 512).cuda()
# flops, params = profile(model.module if hasattr(model, 'module') else model,
#                         inputs=(input_tensor,),
#                         verbose=False)
#
