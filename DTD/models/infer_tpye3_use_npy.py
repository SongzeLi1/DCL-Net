...
device = 'cuda'
n_class = 2
mask_threshold, viz, save_map, save_mask, cal_metrics, batch_size = None, True, True, False, False, 160
input_size = [512, 512]
...
checkpoint_dir = "/pubdata/lisongze/DCLNet/result/DTDNet_crop512x512/detection_type3/dtd_512x512/fold_0_best-ckpt.pth"
save_results_path = '/pubdata/lisongze/DCLNet/result/DTDNet_crop512x512/detection_type3/test_result_915/'
# ---------------
...
for batch_idx, batch_samples in enumerate(test_loader):
    ...
    for i in range(pred.shape[0]):
        ...
        if save_map:
            map = out[i]*255
            np.save(map_path + img_name[:-4] + '.npy', map)
        if save_mask:
            np.save(pred_path + img_name[:-4] + '.npy', pred)
...
metrics_csv = save_metrics_path + "2kinds_metric.csv"
