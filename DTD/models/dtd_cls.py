...
class DTD(SegmentationModel):
    def __init__(self, encoder_name = "resnet18", decoder_channels = (384, 192, 96, 64), classes = 1):
        super().__init__()
        self.vph = torch.load('/pubdata/lisongze/DCLNet/SOTA/DTD/pths/vph_imagenet.pt')
        self.swin = torch.load('/pubdata/lisongze/DCLNet/SOTA/DTD/pths/swin_imagenet.pt')
        self.fph = FPH()
        ...
        self.initialize()
        self.cls = nn.Linear(decoder_channels[-1], 1)
    ...
    def forward(self,x,dct,qt):
        ...
        decoder_output = decoder_output.mean([-2, -1])
        cls = self.cls(decoder_output)
        return cls
