...
min = 0+i*1000
max = 1000+i*1000
save_path = os.path.join("D:\文献阅读\文档类图像检测\工程\数据集\dpreview_dataset\data",str(max))
if not os.path.exists(save_path):
    os.mkdir(save_path)

print('Downloading images from dpreview.com ...')

with open(r"csv\images_" + str(max) + ".csv", mode='w', newline='') as f:
    writer = csv.writer(f)
    for index, img in tqdm(list_images.iterrows(), total=len(list_images)):
        ...
        id_image = img['id']
        if '4084769303' in id_image:
            continue


        if not os.path.exists(save_path + "\\"+str(id_image) + ".jpg"):
            if id_model not in dat_models:
                output_dir = os.path.join(db_root, id_model)
                os.makedirs(output_dir, exist_ok=True)
                page0 = requests.get(
                    "https://www.dpreview.com/sample-galleries/data/get-gallery?galleryId=%s" % id_model)
            ...
            try:
                ...
                continue
            urls = info['url']
            # os.system(r'"D:\迅雷\Thunder\Program\ThunderStart.exe" {urls}'.format(urls=urls))

            print(save_path + str(id_image) + ".jpg")
