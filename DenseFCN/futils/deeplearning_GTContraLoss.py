...
def train_net(fold, logger, param, model, train_data, valid_data):
    ...
    # optimizer = optim.AdamW(model.parameters(), lr=1e-5, weight_decay=5e-4) # ft
    ...
    FocalLoss_fn_cls2 = FocalLoss(mode='binary')
    ...
    # NTXentLoss_fn = losses.NTXentLoss(temperature=0.6)
    temperature = 0.07
    resizesmall4 = Resize([512 // 4, 512 // 4])
    # resizesmall8 = Resize([512 // 8, 512 // 8])
    # resizesmall16 = Resize([512 // 16, 512 // 16])
    # resizesmall32 = Resize([512 // 32, 512 // 32])
    ...
    for epoch in range(epoch_start, epochs):
        ...
        for batch_idx, batch_samples in enumerate(train_loader):
            ...
            # data, gt, binary = batch_samples['image'].cuda(), batch_samples['label'].cuda(), batch_samples['binary'].cuda()
            gt = gt.to(torch.int64) # [b,h,w]
            # binary = binary.to(torch.int64)
            gt01 = gt.clone()
            gt01[gt01 == 2] = 0
            gt4 = resizesmall4(gt01)
            gt4 = gt4.to(torch.int64)  # [b,h,w]
            # gt8 = resizesmall8(gt01)
            # gt16 = resizesmall16(gt01)
            # gt32 = resizesmall32(gt01)
            ...
            with autocast(): #need pytorch > 1.6
                ...
                pred, contra_pred = model(data) # [b,3,h,w] [b,2,h,w]
                # pred, recon = model(data) # 加了文字边缘
                ...
                if param['mixup']:
                    ...
                    # loss_dice = DiceLoss_fn(pred, gt) # dice里面只对target做了onehot->[b,c,h,w] # dice里面多分类会做softmax，二分类会做sigmoid
                    loss_focal = FocalLoss_fn(pred, gt)
                    loss_lovas = LovaszLoss_fn(pred, gt)
                    # loss_dice = DiceLoss_fn(pred, gt)
                    # recon_loss = F.mse_loss(recon, binary)
                    # edge_loss = CrossEntropy_fn(recon, binary)
                    # edge_loss = DiceLoss_fn(recon, binary)

                    # -----计算contrastive loss-------
                    contra_pred = contra_pred.permute(0, 2, 3, 1) # [b,c,h,w]->[b,h,w,c]
                    bz, ht, wd, ns = contra_pred.shape
                    contra_pred = contra_pred.reshape([bz * ht * wd, ns])
                    gt4 = gt4.reshape([bz * ht * wd])  # [allN,1]
                    contra_pred = F.normalize(contra_pred, dim=1)
                    idxpos = torch.where(gt4 == 1)
                    pos = contra_pred[idxpos[0][1:]]  # 注意此处的0不可缺少
                    idxneg = torch.where(gt4 == 0)
                    N = len(pos)
                    # print('pos num:', N)
                    idxneg = idxneg[0][0::10]
                    neg = contra_pred[idxneg[:N]]
                    # print('neg num:', len(neg))
                    q = torch.zeros(N, contra_pred.shape[1])  # N * C
                    q[:] = contra_pred[idxpos[0][0]]
                    # print(q.shape)  # N * C
                    # print(pos.shape) # N * C
                    # print(neg.shape) #  K * C 即 (b-1-N) * C
                    q = q.cuda()
                    l_pos = torch.einsum('nc,nc->n', q, pos).unsqueeze(-1)  # N * 1
                    neg = torch.transpose(neg, 1, 0)
                    l_neg = torch.einsum('nc,ck->nk', q, neg)  # N * K
                    logits = torch.cat([l_pos, l_neg], dim=1)  # N * (1 + K)
                    logits /= temperature
                    labels = torch.zeros(logits.shape[0], dtype=torch.long).cuda()
                    contra_loss = 0.1 * nn.CrossEntropyLoss().cuda()(logits, labels)
                    # ------------------------------

                # loss = loss_focal + (c_loss1 + c_loss2 + c_loss3) / 3
                # loss = 3 * loss_dice + loss_ce
                loss = loss_focal + loss_lovas
                # loss = loss_lovas + contra_loss
                # loss = loss_lovas
                # loss = loss_dice
                # loss = loss_focal + loss_lovas + 0.5 * edge_loss
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_samples in enumerate(valid_loader):
                ...
                # datas, gt3s, _ = batch_samples['image'].cuda(), batch_samples['label'].cuda(), batch_samples['binary'].cuda()
