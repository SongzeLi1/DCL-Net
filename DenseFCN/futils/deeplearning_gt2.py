...
def train_net(fold, logger, param, model, train_data, valid_data):
    ...
    # optimizer = optim.SGD(model.parameters(), lr=1e-1, momentum=0.9, weight_decay=5e-4) # rrunet 训练
    optimizer = optim.SGD(model.parameters(), lr=1e-3, momentum=0.9, weight_decay=5e-4)  # rrunet 微调
    ...
    for epoch in range(epoch_start, epochs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_samples in enumerate(valid_loader):
                datas, gt2s = batch_samples['image'].cuda(), batch_samples['label'].cuda()
                gt2s = gt2s.to(torch.int64)  # [b,h,w]
